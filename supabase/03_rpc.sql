...
-- Corrige el monto/medio de un pago ya registrado, y ajusta su movimiento
-- de caja para que quede igual (evita que la caja quede con el monto
-- viejo). Igual que anular_cobro, si el pago es de antes de guardar el
-- enlace directo, busca el mejor candidato por monto+venta+fecha — y de
-- paso lo enlaza (cobro_id) para que la próxima vez ya sea directo.
-- Devuelve true si encontró y ajustó el movimiento de caja.
...
--   "montos": [3500, 3500, 3500] }   <- OPCIONAL: importes exactos por cliente
--                                        (para redondeo: 10.000/3 = 3.333 c/u,
--                                         pero cada uno paga 3.500). Si viene,
--                                         el total de la venta = suma de montos.
...
begin
  foreach fn in array array[
    ...
    'anular_cobro(bigint,text,text)','editar_cobro(bigint,numeric,text,text)',
